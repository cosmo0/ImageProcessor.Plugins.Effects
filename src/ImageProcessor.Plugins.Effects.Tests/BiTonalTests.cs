        ...
        public void Different_dark_color_yield_different_images()
        {
            foreach (string file in this.images)
            {
                ...
                using (ImageFactory factory = new ImageFactory())
                {
                    ...
                    result2.Save(string.Format("{0}/{1}_bitonal_dark_color2.jpg", Path.GetDirectoryName(file), Path.GetFileNameWithoutExtension(file)));
        ...
        public void Different_light_color_yield_different_images()
        {
            foreach (string file in this.images)
            {
                ...
                using (ImageFactory factory = new ImageFactory())
                {
                    ...
                    Bitmap result = new Bitmap(processor.ProcessImage(factory));
                    Bitmap result2 = new Bitmap(processor2.ProcessImage(factory));
                    result2.Save(string.Format("{0}/{1}_bitonal_light_color2.jpg", Path.GetDirectoryName(file), Path.GetFileNameWithoutExtension(file)));
        ...
        public void Different_threshold_yield_different_images()
        {
            foreach (string file in this.images)
            {
                ...
                using (ImageFactory factory = new ImageFactory())
                {
                    ...
                    Bitmap result = new Bitmap(processor.ProcessImage(factory));
                    Bitmap result2 = new Bitmap(processor2.ProcessImage(factory));
                    result2.Save(string.Format("{0}/{1}_bitonal_threshold2.jpg", Path.GetDirectoryName(file), Path.GetFileNameWithoutExtension(file)));
