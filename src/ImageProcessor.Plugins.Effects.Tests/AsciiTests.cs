        ...
        public void Different_character_count_yield_different_images()
        {
            foreach (string file in this.images)
            {
                ...
                using (ImageFactory factory = new ImageFactory())
                {
                    ...
                    result2.Save(string.Format("{0}/{1}_ascii_character_count2.jpg", Path.GetDirectoryName(file), Path.GetFileNameWithoutExtension(file)), ImageFormat.Jpeg);
        ...
        public void Different_font_size_yield_different_images()
        {
            foreach (string file in this.images)
            {
                ...
                using (ImageFactory factory = new ImageFactory())
                {
                    ...
                    Bitmap result = new Bitmap(processor.ProcessImage(factory));
                    Bitmap result2 = new Bitmap(processor2.ProcessImage(factory));
                    result2.Save(string.Format("{0}/{1}_ascii_fontsize2.jpg", Path.GetDirectoryName(file), Path.GetFileNameWithoutExtension(file)), ImageFormat.Jpeg);
        ...
        public void Different_zoom_yield_different_images()
        {
            foreach (string file in this.images)
            {
                ...
                using (ImageFactory factory = new ImageFactory())
                {
                    ...
                    Bitmap result = new Bitmap(processor.ProcessImage(factory));
                    Bitmap result2 = new Bitmap(processor2.ProcessImage(factory));
                    result2.Save(string.Format("{0}/{1}_ascii_zoom2.jpg", Path.GetDirectoryName(file), Path.GetFileNameWithoutExtension(file)), ImageFormat.Jpeg);
            ...
            foreach (string file in this.images)
            {
                ...
                using (ImageFactory factory = new ImageFactory())
                {
                    ...
                    Action act = () =>
                    {
                        ...
                        img.Save(string.Format("{0}/{1}_ascii_default.jpg", Path.GetDirectoryName(file), Path.GetFileNameWithoutExtension(file)), ImageFormat.Jpeg);
                    };
